#!/usr/bin/env python3
"""Zip + publish ollama gateway to the UEFN Ducky Store via uds_release.
...
Usage:
  ...
  py scripts/release.py --publish --changelog "v1: Ollama gateway"
"""
...
def publish(zip_path: Path, *, category: str, changelog: str) -> None:
    ...
    if not key:
        # Fall back to Cursor mcp.json Bearer for uefn-duckyos-site.
        try:
    ...
    release_args: dict = {
        "category": category,
        "changelog": changelog or "v1: Ollama gateway for Settings → LLMs",
        "publish": True,
        "categories": ["plugins", "gateways"],
        "tags": ["ollama", "local", "llm", "gateway"],
        "name": "Ollama",
        "description": (
            "Local Ollama gateway. After Install + Enable, the Ollama row appears under "
            "Settings → LLMs → Providers & Keys (default URL http://localhost:11434)."
        ),
    }
    if file_id:
